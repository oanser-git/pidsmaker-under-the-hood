...
def feat_inference(indexid2vec, etype2oh, ntype2oh, sorted_paths, out_dir, cfg):
    ...
    for path in log_tqdm(sorted_paths, desc="Computing edge embeddings"):
        ...
        if cfg.feat_inference.sort_edges_by_time:
            edges.sort(key=temporal_edge_sort_key)

        src, dst, msg, t, y, event_order = [], [], [], [], [], []
        for u, v, k, attr in edges:
            src.append(int(u))
            dst.append(int(v))
            t.append(int(attr["time"]))
            y.append(int(attr.get("y", 0)))
            event_order.append(next_event_order)
            next_event_order += 1
        ...
        data = CollatableTemporalData(
            ...
            event_order=torch.tensor(event_order).to(torch.long),
        )
...
def temporal_edge_sort_key(edge):
    u, v, key, attr = edge
    return (
        int(attr["time"]),
        str(attr.get("event_uuid", "")),
        str(u),
        str(v),
        str(attr.get("label", "")),
        str(key),
