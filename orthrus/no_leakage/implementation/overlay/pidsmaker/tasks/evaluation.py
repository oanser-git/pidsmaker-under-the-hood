...
from pidsmaker.utils.data_utils import file_sha256, load_model
...
def standard_evaluation(cfg, evaluation_fn):
    ...
    if cfg.evaluation.selected_epoch >= 0:
        sorted_files = [f"model_epoch_{cfg.evaluation.selected_epoch}"]
    else:
        sorted_files = (
...
def infer_selected_checkpoint(cfg):
    epoch = cfg.evaluation.selected_epoch
    checkpoint_dir = os.path.join(cfg.training._trained_models_dir, f"model_epoch_{epoch}")
    state_dict_path = os.path.join(checkpoint_dir, "state_dict.pkl")
    if not os.path.isfile(state_dict_path):
        raise FileNotFoundError(
            f"Selected checkpoint does not exist: {state_dict_path}. "
            "Final evaluation never retrains a missing model."
        )

    expected_sha256 = cfg.evaluation.expected_checkpoint_sha256
    actual_sha256 = file_sha256(state_dict_path)
    ...
    model = load_model(model, checkpoint_dir, cfg, map_location=device)

    model_epoch_dir = f"model_epoch_{epoch}"
    for split in ["val", "test"]:
        shutil.rmtree(
            os.path.join(cfg.training._edge_losses_dir, split, model_epoch_dir),
            ignore_errors=True,
        )
    ...
    if file_sha256(state_dict_path) != expected_sha256:
        raise RuntimeError("Selected checkpoint changed during final evaluation.")
    wandb.log(
        {
            "selected_epoch": epoch,
            "selected_checkpoint": checkpoint_dir,
