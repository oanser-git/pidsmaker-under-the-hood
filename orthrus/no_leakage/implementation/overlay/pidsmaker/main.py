...
def get_pipeline_tasks(cfg):
    tasks = list(get_task_to_module(cfg).keys())
    if cfg.evaluation.selected_epoch >= 0:
        return ["evaluation"]
    if cfg.training.validation_only:
        return [task for task in tasks if task not in ["evaluation", "triage"]]
    return tasks
...
def main(cfg, project=None, exp=None, sweep_id=None, **kwargs):
    ...
    def run_pipeline(cfg, method=None, iteration=None):
        ...
        """
        tasks = get_pipeline_tasks(cfg)
        task_results = {task: run_task(task, cfg, method, iteration) for task in tasks}

        metrics = task_results.get("evaluation", {}).get("return") or {}
        if "training" in task_results:
