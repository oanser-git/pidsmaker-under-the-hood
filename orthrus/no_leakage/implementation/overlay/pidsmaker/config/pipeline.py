...
def set_task_paths(cfg, subtask_concat_value=None):
    ...
    for task in TASK_ARGS:
        ...
        if task == "construction":
            restart_values += [
                f"train_dates={cfg.dataset.train_dates}",
                f"val_dates={cfg.dataset.val_dates}",
                f"test_dates={cfg.dataset.test_dates}",
                f"ground_truth={cfg.dataset.ground_truth_relative_path}",
                f"attack_windows={cfg.dataset.attack_to_time_window}",
            ]
            if (
...
def get_yml_cfg(args):
    ...
    resolve_derived_config_values(cfg)
...
def resolve_derived_config_values(cfg):
...
def check_edge_cases(cfg):
    ...
    if cfg.evaluation.selected_epoch < -1:
        raise ValueError("`evaluation.selected_epoch` must be -1 or a non-negative epoch.")
    if cfg.evaluation.selected_epoch >= 0 and not cfg.evaluation.expected_checkpoint_sha256:
        raise ValueError("Selected-epoch evaluation requires `expected_checkpoint_sha256`.")
    if cfg.training.validation_only and cfg.training_loop.run_evaluation != "each_epoch":
        raise ValueError("Validation-only training requires `training_loop.run_evaluation=each_epoch`.")

    if cfg.construction.enforce_chronological_splits:
        validate_chronological_splits(cfg)
    ...
    if use_tgn_neigh_loader:
        ...
        tgn_cfg = cfg.batching.intra_graph_batching.tgn_last_neighbor
        if tgn_cfg.assert_causal_neighbors:
            if tgn_cfg.insert_neighbors_before:
                raise ValueError(
                    "Causal TGN validation requires `insert_neighbors_before=False`."
                )
            if not cfg.feat_inference.sort_edges_by_time:
                raise ValueError(
                    "Causal TGN validation requires `feat_inference.sort_edges_by_time=True`."
                )
            if not cfg.construction.enforce_chronological_splits:
                raise ValueError(
                    "Causal TGN validation requires chronological train/validation/test splits."
                )
...
def validate_chronological_splits(cfg):
    split_dates = {
        "train": [date.fromisoformat(value) for value in cfg.dataset.train_dates],
        "validation": [date.fromisoformat(value) for value in cfg.dataset.val_dates],
        "test": [date.fromisoformat(value) for value in cfg.dataset.test_dates],
    }

    if any(not values for values in split_dates.values()):
        raise ValueError("Strict chronological mode requires non-empty train, validation, and test splits.")
    ...
    if set(train) & set(validation) or set(train) & set(test) or set(validation) & set(test):
        raise ValueError("Train, validation, and test dates must be disjoint.")
    if max(train) >= min(validation) or max(validation) >= min(test):
        raise ValueError(
            "Strict chronological mode requires max(train) < min(validation) < min(test)."
        )
