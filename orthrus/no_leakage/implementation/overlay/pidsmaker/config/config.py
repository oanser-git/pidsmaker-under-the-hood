...
TASK_ARGS = {
    "construction": {
        ...
        "enforce_chronological_splits": Arg(
            bool,
            desc="Require every training date to precede validation and every validation date to precede test.",
        ),
        "node_label_features": {
            "subject": Arg(
                str,
    ...
    "feat_inference": {
        ...
        "sort_edges_by_time": Arg(
            bool,
            desc="Sort transformed edges by timestamp and a stable event key before temporal batching.",
        ),
    },
    "batching": {
        "save_on_disk": Arg(
        ...
        "intra_graph_batching": {
            ...
            "tgn_last_neighbor": {
                ...
                "assert_causal_neighbors": Arg(
                    bool,
                    desc="Fail if a TGN context edge occurs after the batch it is used to predict.",
                ),
            },
        },
    ...
    "training": {
        ...
        "validation_only": Arg(
            bool,
            desc="During training, load and infer only on train/validation data and do not generate test scores.",
        ),
        "deterministic": Arg(
    ...
    "evaluation": {
        "selected_epoch": Arg(
            int,
            desc="Load and evaluate exactly this saved epoch; -1 uses the standard multi-epoch evaluation.",
        ),
        "expected_checkpoint_sha256": Arg(
            str,
            desc="Required SHA-256 digest of the frozen checkpoint used for selected-epoch evaluation.",
        ),
