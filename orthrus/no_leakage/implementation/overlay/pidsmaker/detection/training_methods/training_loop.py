...
from pidsmaker.utils.data_utils import file_sha256, save_model
...
def main(cfg):
    ...
    train_data, val_data, test_data, max_node_num = get_preprocessed_graphs(
        cfg, include_test=not cfg.training.validation_only
    )
    ...
    best_val_loss_model_path = None
    best_val_loss_model_sha256 = None
        ...
        if (epoch + 1) % 2 == 0 or epoch == 0:
            ...
            if validation_only:
                model_path = os.path.join(
                    cfg.training._trained_models_dir, f"model_epoch_{epoch}"
                )
                save_model(model, model_path, cfg)
                model_sha256 = file_sha256(os.path.join(model_path, "state_dict.pkl"))
            ...
            test_stats = inference_loop.main(
                ...
                test_data=test_data,
                epoch=epoch,
                split="val" if validation_only else "all",
            )
            all_test_stats.append(test_stats)

            val_loss = float(test_stats["val_loss"])
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                best_val_loss_epoch = epoch
                best_val_loss_model_path = model_path
                best_val_loss_model_sha256 = model_sha256
            ...
            if model_path is not None:
                log_payload["model_path"] = model_path
                log_payload["model_sha256"] = model_sha256
            if test_stats["test_loss"] is not None:
                log_payload["test_loss"] = round(test_stats["test_loss"], 4)
            wandb.log(log_payload)

    # After training
    if best_epoch_mode and not validation_only:
        model.load_state_dict(best_model)
        test_stats = inference_loop.main(
            cfg=cfg,
            model=model,
    ...
    wandb.log(
        {
            ...
            "best_val_loss_model_path": best_val_loss_model_path,
            "best_val_loss_model_sha256": best_val_loss_model_sha256,
