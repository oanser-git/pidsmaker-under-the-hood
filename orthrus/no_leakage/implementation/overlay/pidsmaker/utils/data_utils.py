...
def load_all_datasets(cfg, device, only_keep=None, include_test=True):
    ...
    test_data = load_data_set(cfg, split="test", multi_dataset=False) if include_test else []
    ...
    dataset_groups = [train_data, val_data]
    if include_test:
        dataset_groups.append(test_data)
    full_data = get_full_data(dataset_groups)
...
def compute_tgn_graphs(
    ...
    assert_causal_neighbors = tgn_loader_cfg.assert_causal_neighbors
    ...
    for dataset in datasets:
        for data_list in dataset:
            for batch in log_tqdm(data_list, desc="Computing TGN last neighbor graphs"):
                ...
                batch.event_order_tgn = full_data.event_order[e_id.cpu()]
                if assert_causal_neighbors:
                    assert_causal_neighbor_times(
                        batch.t_tgn,
                        batch.event_order_tgn,
                        batch.t,
                        batch.event_order,
                    )
...
def assert_causal_neighbor_times(
    neighbor_times,
    neighbor_order,
    current_times,
    current_order,
):
    if neighbor_times.numel() == 0 or current_times.numel() == 0:
        return

    # TGN context attributes are indexed from CPU-backed full_data while the
    # current batch may already be on CUDA. This assertion does not need GPU.
    neighbor_times = neighbor_times.detach().cpu()
    neighbor_order = neighbor_order.detach().cpu()
    current_times = current_times.detach().cpu()
    current_order = current_order.detach().cpu()

    earliest_time = current_times.min()
    earliest_order = current_order[current_times == earliest_time].min()
    future_time = neighbor_times > earliest_time
    invalid_equal_time = (neighbor_times == earliest_time) & (neighbor_order >= earliest_order)
    if bool((future_time | invalid_equal_time).any().item()):
        latest_neighbor = int(neighbor_times.max().item())
        earliest_current = int(earliest_time.item())
        raise ValueError(
            "Non-causal TGN context detected: "
            f"neighbor timestamp/order does not precede the current batch beginning at "
            f"timestamp {earliest_current}; latest neighbor timestamp is {latest_neighbor}."
        )
...
def save_model(model, path: str, cfg):
    ...
    if isinstance(model.encoder, TGNEncoder) and hasattr(model.encoder, "neighbor_loader"):
        torch.save(
            model.encoder.neighbor_loader,
            os.path.join(path, "neighbor_loader.pkl"),
            pickle_protocol=pickle.HIGHEST_PROTOCOL,
        )
    if isinstance(model.encoder, TGNEncoder) and (
